...
class ProcRTL( Component ):
  ...
  def construct( s ):
    ...
    connect_pairs(
      ...
      s.ctrl.mask_sel_W     , s.dpath.mask_sel_W,
  ...
  def line_trace( s ):
    ...
    # D stage
    if not s.ctrl.val_D:  D_str = "{:<6s}".format( ' ' )
    elif s.ctrl.squash_D: D_str = "{:<6s}".format( '~' )
    elif s.ctrl.stall_D:  D_str = "{:<6s}".format( '#' )
    else:                 D_str = "{:<6s}".format( disassemble_inst(s.ctrl.inst_D) )
