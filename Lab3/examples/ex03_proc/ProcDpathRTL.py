...
from .MiscRTL import AluRTL, ImmGenRTL, MaskRTL
...
class ProcDpath( Component ):
  ...
  def construct( s ):
    ...
    # forward declaration for branch target and jal target
    ...
    # PC sel mux
    s.pc_sel_mux_F = Mux( Bits32, 3 )(
      in_ = { 0: s.pc_plus4_F, 1: s.br_target_X, 2: s.bypass_X },
      sel = s.pc_sel_F,
      out = s.imemreq_addr,
    )
    ...
    s.pc_reg = Wire( Bits32 )
    s.alu_out = Wire(Bits32)
    s.pc_reg_D = m = RegEnRst( Bits32 )( en = s.reg_en_D, in_ = s.pc_F, out = s.pc_reg )
    ...
    s.mask_W = MaskRTL()(
      in0 = s.mask_in,
      sel = s.mask_sel_W,
      out = (s.rf_wdata_W, s.proc2mngr_data)
    )