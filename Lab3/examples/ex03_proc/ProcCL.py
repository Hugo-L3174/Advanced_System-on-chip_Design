            ...
            if s.F_DXM_queue.deq.rdy() and s.imemresp_q.deq.rdy():
                ...
                if not s.DXM_W_queue.enq.rdy():
                    ...
                    if inst_name == "nop":
                        ...
                        s.DXM_W_queue.enq((inst.rd, s.R[inst.rs1].int() < s.R[inst.rs2].int(), DXM_W.arith))
                    elif inst_name == "sltu":
                        s.DXM_W_queue.enq((inst.rd, s.R[inst.rs1] < s.R[inst.rs2], DXM_W.arith))
                    elif inst_name == "sra":
                        s.DXM_W_queue.enq((inst.rd, s.R[inst.rs1].int() >> (s.R[inst.rs2].uint() & 0x1F), DXM_W.arith))
                    elif inst_name == "srl":
                    ...
                    # ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''/\
                        ...
                        s.DXM_W_queue.enq((inst.rd, s.R[inst.rs1].int() >> (inst.i_imm.uint() & 0x1F), DXM_W.arith))
                    elif inst_name == "slti":
                        s.DXM_W_queue.enq((inst.rd, s.R[inst.rs1].int() < inst.i_imm.int(), DXM_W.arith))
                    elif inst_name == "sltiu":
                        s.DXM_W_queue.enq((inst.rd, s.R[inst.rs1] < sext(inst.i_imm,32), DXM_W.arith))

                    elif inst_name == "auipc":
                      s.DXM_W_queue.enq((inst.rd, (s.pc-4) + inst.u_imm, DXM_W.arith))  #not elegant
                    elif inst_name == "lui":
                      s.DXM_W_queue.enq((inst.rd, inst.u_imm, DXM_W.arith))
                        ...
                        if s.dmem.req.rdy():
                            s.dmem.req(memreq_cls(MemMsgType.READ, 0, s.R[inst.rs1] + inst.i_imm.int(), 1))
                        ...
                        if s.dmem.req.rdy():
                            s.dmem.req(memreq_cls(MemMsgType.READ, 0, s.R[inst.rs1] + inst.i_imm.int(), 2))
                            s.DXM_W_queue.enq((inst.rd, 0, DXM_W.mem_signed))
                        else:
                            s.DXM_status = PipelineStatus.stall
                        ...
                        s.DXM_W_queue.enq((inst.rd, s.pc , DXM_W.arith))
                        s.pc = (s.R[inst.rs1] + sext(inst.i_imm, 32)) & 0xFFFFFFFE
            ...
            if s.DXM_W_queue.deq.rdy():
                ...
                if entry is not None:
                    ...
                    if entry_type == DXM_W.mem:
                        ...
                        if s.dmemresp_q.deq.rdy():
                            if rd > 0:  # load
                                s.R[rd] = Bits32(sext(s.dmemresp_q.deq().data,32))
                            else:  # store
                                s.dmemresp_q.deq()
